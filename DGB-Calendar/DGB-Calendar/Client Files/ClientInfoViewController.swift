  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "addDogSegue" {
      ...
      if let editVC = destinationNavController.topViewController as? AddDogViewController {
        editVC.newDog = true
        editVC.client = client
        editVC.clientsDogs = clientsDogs
    ...
    if segue.identifier == "viewDogInfoSegue" {
      ...
      if let viewVC = destinationNavController.topViewController as? DogInfoViewController {
        ...
        viewVC.client = client
      }
    }
    if segue.identifier == "doneViewingClientInfo" {
      if let viewVC = segue.destination as? ClientViewController {
...
extension ClientInfoViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    if indexPath.section == 0 {
      ...
      cell.textLabel?.text = clientsDogs[indexPath.row].value(forKey: "Name") as! String
      return cell
    }
  }
  ...
  func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
    let dogIDToRemove = clientsDogs[indexPath.row].recordID
    database.delete(withRecordID: dogIDToRemove) { (records, _) in
      guard records != nil else { return }
      self.clientsDogs.remove(at: indexPath.row)
      for (index, dog) in self.dogRecords!.enumerated() {
        if dog.recordID == dogIDToRemove {
